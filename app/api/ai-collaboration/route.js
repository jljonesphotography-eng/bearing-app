...
amplification_methods (exactly 3 strings): Each method must be written in plain coaching language — as if a trusted advisor is speaking directly to the person. No technical jargon. No product names unless they are widely known (like Claude or ChatGPT). No buzzwords like 'deploy,' 'leverage,' or 'implement.' Start each method with what the person actually does, not what tool they use. Example: 'When you're preparing for a difficult conversation, use Claude to help you map out the different perspectives in the room before you walk in — this extends your natural ability to read dynamics before they surface.' The person should finish reading the method and think: I could do that this week.`;

function buildUserMessage({ verdict, primary_finding, zone, energy_profile }) {
  return `Verdict: ${verdict}. Primary finding: ${primary_finding}. Zone: ${zone}. Engagement profile: ${energy_profile}. Generate a personalized AI collaboration guide as JSON with exactly these keys: domain_shift (one paragraph — where AI is already operating in this person's domain based on their primary finding, what they should stop investing energy in), amplification_methods (array of exactly 3 strings — follow the system prompt rules for plain coaching language; each concrete and specific to this person), next_skill (one sentence — the single most important skill to build in the next 30 days based on what the assessment found).`;
}
